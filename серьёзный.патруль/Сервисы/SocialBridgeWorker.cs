...
public class SocialBridgeWorker
{
    ...
    public SocialBridgeWorker(Сеть.КлиентПатруля клиент, КонфигурацияПатруля конфигурация)
    {
        this.клиент = клиент;
    ...
    public async Task ЗапуститьAsync(CancellationToken токен)
    {
        while (!токен.IsCancellationRequested)
        {
            try
            {
                if (клиент.Подключен)
                {
                    var запрос = SocialBridgeService.TakeNextPending();

                    if (запрос != null)
                        await ОбработатьЗапросAsync(запрос.Value, токен);
                }

                SocialBridgeService.Cleanup(TimeSpan.FromMinutes(10));
            }
            catch (Exception ошибка)
            {
                серьёзный.патруль.Сервисы.Лог.Записать("SocialBridgeWorker: " + ошибка);
            }
    ...
    private async Task ОбработатьЗапросAsync(
        (long Id, Guid AccountId, серьёзный.Core.CoreModels.SocialActionDto Action) запрос,
        CancellationToken токен)
    {
        ...
        {
            Команда = серьёзный.Сеть.КомандаПК.ЗапроситьСоциальныйСтатус,
            АккаунтId = запрос.AccountId,
            Параметры = System.Text.Json.JsonSerializer.Serialize(запрос.Action)
        });
        ...
        try
        {
            ...
            var данные = ответ.ПолучитьДанные<серьёзный.Core.CoreModels.SocialStateDto>();

            if (данные != null)
                SocialBridgeService.CompleteRequest(запрос.Id, данные);
        }
        catch (OperationCanceledException)
        {
            ожидание.TryRemove(сообщение.ИдентификаторСообщения, out _);
