    ...
    public partial class ОкноСеанса001 : Window
    {
        ...
        private void Начать(object sender, RoutedEventArgs e)
        {
            ...
            if (Аккаунт == null)
            {
                ...
                var телефон = (ПолеТелефонНовогоАккаунта.Text ?? string.Empty).Trim();

                if (string.IsNullOrWhiteSpace(телефон))
                {
                    MessageBox.Show("Для нового аккаунта необходимо указать телефон.");
                    ПолеТелефонНовогоАккаунта.Focus();
                    return;
                }

                if (!сервис.Создать(телефон, пароль, имя, out var ошибка))
                {
                    MessageBox.Show(ошибка);
                    return;
                }

                Аккаунт = сервис.НайтиПоТелефону(телефон);
