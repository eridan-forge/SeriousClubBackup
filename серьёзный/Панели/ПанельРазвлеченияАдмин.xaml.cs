...
public partial class ПанельРазвлеченияАдмин : UserControl
{
    ...
    private readonly серьёзный.Core.CoreShop.SessionTariffService tariffs = new();
    private readonly CurrencyVisibilityService видимость = new();
    ...
    private void ОбновитьВсё()
    {
        ...
        ГридТарифы.ItemsSource = null;
        ГридТарифы.ItemsSource = tariffs.GetAll();
        
        var текущаяВидимость = видимость.Get();
        ФлагПоказыватьБаллы.IsChecked = текущаяВидимость.ShowPoints;
        ФлагПоказыватьСтоимость.IsChecked = текущаяВидимость.ShowSessionCost;
    }
    ...
    private void ДобавитьТариф_Click(object sender, RoutedEventArgs e)
    {
        tariffs.Save(new серьёзный.Core.CoreShop.SessionTariff
        {
            Minutes = 60,
            Price = 150,
            Label = "Новый тариф",
            SortOrder = tariffs.GetAll().Count
        });

        ОбновитьВсё();
    }

    private void СохранитьТарифы_Click(object sender, RoutedEventArgs e)
    {
        foreach (var t in ГридТарифы.ItemsSource.Cast<серьёзный.Core.CoreShop.SessionTariff>())
            tariffs.Save(t);

        MessageBox.Show("Тарифы сохранены.");
    }

    private void УдалитьТариф_Click(object sender, RoutedEventArgs e)
    {
        if (ГридТарифы.SelectedItem is not серьёзный.Core.CoreShop.SessionTariff t)
            return;
    ...
    private void СохранитьВидимостьВалют_Click(object sender, RoutedEventArgs e)
    {
        видимость.Save(new CurrencyVisibility
        {
            ShowPoints = ФлагПоказыватьБаллы.IsChecked == true,
            ShowSessionCost = ФлагПоказыватьСтоимость.IsChecked == true
        });

        MessageBox.Show("Видимость валют сохранена.");
    }
}